-- Complete Database Schema Fix for Explore Kaltara
...
-- =================================
-- STEP 2: Create/Fix Tables with Correct Foreign Keys
-- =================================
...
-- Drop existing bookings table and recreate with correct foreign keys
...
-- Drop existing reviews table and recreate with correct foreign keys
...
CREATE TABLE public.reviews (
  ...
  status TEXT CHECK (status IN ('pending', 'approved', 'rejected')) DEFAULT 'pending',  admin_notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL,
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT TIMEZONE('utc'::text, NOW()) NOT NULL
);
...
-- Success message
SELECT 'Database schema has been successfully updated!' as status;
